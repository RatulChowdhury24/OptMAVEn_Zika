...
def CHARMM_PDB_position(group, experiment):
    ...
    # experiment input contains position restraints and that the group is an
    # Objective Group.
        ...
        for molecule in molecules:
            ...
            for residue in residues:
                ...
                for atom in atoms:
                    ...
                    if atom.name in residue.fixedAtoms:
                        continue
...
def position_restraints(molecules, experiment):
    ...
    # Use a try statement to catch any problems
    try:
        # Validate the inputs. Molecules must be an objective group
        if not isinstance(molecules, MOLECULES.ObjectiveGroup):
            return ''
        # Make sure there are position restraints in the Experiment
        if "Restraints" not in experiment or "Position" not in \
        experiment["Restraints"]:
            return ''
        # If the experiment some how does not have file format or force field
        # information
        if "File Format" not in experiment or "Force Field" not in experiment:
            return ''
        # If the CHARMM force field and the PDB file format are being used
        if experiment["Force Field"] == "CHARMM" and experiment["File Format"] \
        == "PDB":
            return CHARMM_PDB_position(molecules, experiment)
        # Otherwise raise an error that the file format and force field are not
        # supported
        else:
            ...
            raise RestraintError(text)
    # If there was a type or attribute error because the proper information
    # could not be accessed from the experiment input, return ''
    except (TypeError, AttributeError, KeyError):
        return ''

def CHARMM_PDB_distance(group, experiment):
    """Create restraints that maintain the distance between two Atoms"""
    # The distance_restraints function has confirmed that the inputs are valid
    # and that this function should be called
...
def distance_restraints(molecules, experiment):
    ...
    # Use a try statement to catch errors that arise from experiment not having
    # the proper form
    try:
        # Validate the inputs. Molecules must be an Objective Group
        if not isinstance(molecules, MOLECULES.ObjectiveGroup):
            return ''
        # Make sure that the experiment contains all of the inputs it needs
        if "File Format" not in experiment or "Force Field" not in experiment \
        or "Restraints" not in experiment or "Distance" not in \
        experiment["Restraints"]:
            return ''
        # If the PDB file format and the charmm force field are being used
        if experiment["Force Field"] == "CHARMM" and experiment["File Format"] \
        == "PDB":
            return CHARMM_PDB_distance(molecules, experiment)
        # Otherwise raise an error because the file format / force field
        # combination is not supported
            ...
            text += str(experiment["Force Field"]) + " force field combination"
            raise RestraintError(text)
    # If the experiment does not have the proper form to access the information,
...
def CHARMM_PDB_dihedral(group, experiment):
    ...
    # The dihedral restraints function has already confirmed that the group and
    # experiment inputs are acceptable and that this particular function is the
    # one that should have been called
    # Store the restraints here
    dihedral = ''
...
def dihedral_restraints(molecules, experiment):
    ...
    # Use a try statement only to catch errors that may arise from experiment
    # not being a container of information
    try:
        # Validate that molecules is an Objective Group
        if not isinstance(molecules, MOLECULES.ObjectiveGroup):
            return ''
        # Make sure that the experiment has all of the information it needs
        if "File Format" not in experiment or "Force Field" not in experiment \
        or "Restraints" not in experiment or "Dihedral" not in \
        experiment["Restraints"]:
            return ''
        # If the PDB File format and the CHARMM force field are being used
        if experiment["File Format"] == "PDB" and experiment["Force Field"] \
        == "CHARMM":
            return CHARMM_PDB_dihedral(molecules, experiment)
        # Otherwise raise an error because the force field and file format
        # combination is not supported
        else:
            ...
            text += str(experiment["Force Field"]) + " force field combination."
            raise RestraintError(text)
    # If there were any errors because the experiment input is not of the proper
    # form to contain the desired information, return a blank string
