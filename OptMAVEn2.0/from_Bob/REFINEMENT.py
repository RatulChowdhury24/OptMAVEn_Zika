...
def ensemble_refinement(experiment, gn, en):
    """Do a refinement of the specified Objective Group"""
...
def first_group_refinement(experiment):
    """Start all ensembles for the Objective Groups"""
    # Start sharing (for file creation reasons)
    SHARING.Start(experiment)
    # Loop through the groups
    for group in experiment:
        gn = group.number
        # Generate the name for the group's calculations
        folder = "Group" + str(gn)
        try:
            os.mkdir(folder)
        except OSError:
...
def second_group_refinement(experiment):
    """Finish all ensembles of all Objective Groups"""
    # Loop through the groups
    for group in experiment:
        gn = group.number
        # Move into the folder
...
def make_extra_group(experiment):
    """Create an extra Objective Group with no Target Molecules"""
    # Only do this if Binding energy calculations are being done
    if experiment["Energy Calculation"] == "Binding":
        # Get all of the Design Molecules from Group 1
        molecules = []
        for molecule in experiment[1]:
            if molecule.design:
                molecules.append(molecule)
        # Make and store a new Group (which duplicates these Molecules
...
def delete_extra_group(experiment):
    """Delete the extra objective group made for binding calculations"""
    if experiment["Energy Calculation"] == "Binding":
        N = len(experiment)
...
def gather_energies(experiment):
    ...
    # Store them here
    energies = {}
    # Go through the Objective Groups
    for group in experiment:
...
def calculate_IEs(energies):
    ...
    for gn in energies:
        ...
        if "Interaction" not in energies[gn][1]:
            continue
        # Make a list of the interaction energies from each Group
        values = []
        ...
        # Store that info for the Group
        IEs[gn] = {"Average":[average, sd]}
        # Find the Group that is closest to the average value
        best = None
        difference = 5000000
        for en in energies[gn]:
...
def calculate_CEs(energies):
    """Calculate the average complex energies of the Objective Groups"""
...
def calculate_BEs(experiment, energies):
    ...
    if experiment["Energy Calculation"] == "Binding":
        # Collect the Complex energies from the last Objective Group
        N = len(experiment)
        designs = []
        for en in energies[N]:
            designs.append(energies[N][en]["Complex"])
        # Go through the other Objective Groups
        for gn in energies:
            # Skip the last group
            if gn == N:
                continue
...
def make_refinement_choice(experiment, IEs, BEs):
    ...
    for group in experiment:
        ...
        if group.objective in ['improve', 'maintain']:
            ...
            if energy < 0:
                return False
    # if all Objective Groups passed the test, return True
    return True
...
def store_results(experiment, IEs, BEs, CEs, mn = None):
    ...
    if experiment["Type"] != "Mutator":
        ...
        mn = int(f.readline())
        f.close()
    # Go through the Objective Groups, identify the chosen ensemble, and store the
...
def Summarize(experiment, choice, IEs, BEs, CEs, mn = None):
    ...
    for group in experiment:
        ...
        experiment["Summary"] += "Objective Group " + str(group.number) + "\n"
