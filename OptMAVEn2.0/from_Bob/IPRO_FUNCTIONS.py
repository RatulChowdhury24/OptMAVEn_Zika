...
def do_superimpose(experiment):
    ...
    if "Restraints" in experiment and "Position" in experiment["Restraints"]:
        for restraint in experiment["Restraints"]["Position"]:
            ...
            # Objective Groups, not to the initial positions, and can be skipped
            if isinstance(restraint[-1], int):
                continue
            # Make a list of the Objective Groups the restraint applies to
            gns = []
            if restraint[0] == 'all':
                for group in experiment:
...
def Closest_Rotamers(experiment, gn = None):
    ...
    # Start timing this
    startTime = time.time()
    # Loop through the Objective Groups
    for group in experiment:
...
def Backbone_Perturbation(experiment, gn = None):
    ...
    for group in experiment:
        ...
        if gn == None:
            experiment["Summary"] += SHARING.summary_update(groupTime, \
                        "Perturbing Objective Group " + str(group.number))
...
def Relaxation(experiment, gn = None, all = True):
    ...
    # Start timing
    startTime = time.time()
    # Loop through the relevant Objective Groups
    for group in experiment:
        if gn not in [None, group.number]:
            continue
        # Time the group
        ...
        if gn == None:
            experiment["Summary"] += SHARING.summary_update(groupTime, \
            "The Relaxation of Objective Group " + str(group.number))
    experiment["Summary"] += SHARING.summary_update(startTime, \
...
def Calculate_Energy(experiment, gn = None):
    ...
    for group in experiment:
        ...
        if gn == None:
            experiment["Summary"] += SHARING.summary_update(groupTime, \
            "Energy Calculation for Objective Group " + str(group.number))
...
def Optimal_Rotamers(experiment, gn = None):
    ...
    if gn == None:
        ...
        N = gn
    # Do this for each Objective Group
    for group in experiment:
        if gn not in [None, group.number]:
            continue
        # Time the group
        groupTime = time.time()
        # Make sure the amino acid sequences match across Objective Groups
        if group.number != N:
            for molecule in group:
                for residue in molecule:
        ...
        # Pick the Rotamers
        probability = True
        objective, solution = ROTAMERS.Optimal_Rotamers(group, experiment, \
                                                        probability)
        ...
        message += "The objective value of the MILP was " + format(objective, \
                   '.3f') + "\n"
        experiment["Summary"] += SHARING.summary_update(groupTime, \
        "Selecting Rotamers for Objective Group " + str(group.number), message)
    return refinement
...
def store_structures(experiment, gn = None):
    """Store the structures of the Experiment's Objective Groups"""
    # Loop through the Objective Groups
    for group in experiment:
        if gn not in [None, group.number]:
            continue
...
def finish_initialization(experiment):
    ...
    for line in f:
        if line.startswith("Started"):
            ...
            break
    f.close()
    # Include the energies of each Objective Group
    for group in experiment:
