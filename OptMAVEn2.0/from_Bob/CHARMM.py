...
# Include the classes from the MOLECULES module and the Solvation and Restraints
# modules
import MOLECULES
import SOLVATION
import RESTRAINTS
...
def validate_molecules(molecules):
    ...
    # The input should be either an Objective Group, a Molecule, or a list of
    # Molecules. If it is an Objective Group, retrieve the group's number.
    error = False
    gn = None
    # molecules can be one of three things: an Objective Group
    if isinstance(molecules, MOLECULES.ObjectiveGroup):
        gn = molecules.number
    # A Molecule
    ...
    if error:
        text = "The functions in the CHARMM module will work with either an "
        text +="ObjectiveGroup, Molecule, or list of Molecules as input, but "
        text += "not:\n" + str(molecules)
        raise CHARMM_Error(text)
    ...
    # Return the pointer to the molecules and the group number
    return molecules, gn
...
def introduction(purpose = None):
    ...
    # Set the options for CHARMM: the warning, bomb, and print levels all to -2,
    # but the print level is optioned out (It has always been that way in IPRO,
    # so I'm leaving it for nostalgia's sake)
    text += "\n*\n\nwrnl -2\n!prnl -2\nbomb -2\n\n"
    return text
...
def load_input_files(experiment = None):
    ...
    for set in sets:
        ...
        for F in files:
            # Open the file
            output += "open read unit 10 form name " + F + " card\n"
...
def Missing_Atoms(molecules, experiment = None):
    ...
    # Declare the procedure, making sure it is OK (it is, but checking it is
    # easy)
...
def Energy(molecules, experiment = None, which = "all"):
    ...
    if isinstance(gn, int):
        script = introduction("Energy Calculation for Objective Group "+str(gn))
    else:
