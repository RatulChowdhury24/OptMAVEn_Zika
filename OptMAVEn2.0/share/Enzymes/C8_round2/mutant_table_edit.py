...
import os
import sys
sys.path.append("/gpfs/work/npg115/IPRO_Suite/modules/")
...
def mutant_info(mutants, folder, groups, sequences, design_positions):
    ...
    if sequence in sequences:
        ...
        mutants[key]["iter"].append(folder[9:])
        ...
        "instances": 1, "complex average": {},"bestiter": {},"iter": []}
        mutants[key]["bestiter"] = folder[9:]
        mutants[key]["iter"].append(folder[9:])
        for group in group_keys:
            IE, CE = gather_energy_data(folder, group)
...
sequences = []
os.chdir(path2 + folder)
mutant_info(mutants, "wildtype", dg, sequences, design_positions)
list = os.listdir(".")
List = []
for file in list:
    if file[:6] == "mutant":
        List.append(int(file[6:]))
List.sort()
print List
for i in List:
    mutant_info(mutants, "mutant" + str(i), dg, sequences, design_positions)
os.chdir(current) 
...
text += "Instances,Best Iteration,Iterations \n"
...
for key in mutant_keys:
    ...
    for group in group_keys:
        ...
        number) + ","
    text += str(number) + ","       
    text += mutants[key]["bestiter"] + ","
    for i in mutants[key]["iter"]:
        text += i + ","
    text += "\n"

with open("Mutant_Information_edit.csv", "w") as file:
    file.write(text)
