...
def Molecules(experiment, data):
    ...
    if "Molecule" not in data:
        ...
        for line in data["Molecule"]:
            ...
            # Load the Molecule
            try:
                f, fileName = CHECK.for_file(fileName, "./", True)
...
#mfa#
def OptmavenGrid(experiment, data):
    """ Load the OptMAVEn grid. """
    errors = ""
    if "Optmaven Grid" not in data:
        errors += "\nThere is no OptMAVEn grid information."
    else:
        grid = data["Optmaven Grid"]
        # The grid should have four dimensions.
        if len(grid) != 4:
            errors += "\nThe OptMAVEn grid has {} dimensions; it should have 4"\
                    .format(len(grid))
        # Every entry should be a floating-point number.
        try:
            grid = [[float(x) for x in dimension] for dimension in grid]
        except ValueError:
            errors += "\nEvery entry in the OptMAVEn grid must be a number."
        # Store the information.
        if errors == "":
            experiment["Optmaven Grid"] = grid
    return errors
...
def OptzymeGroups(experiment, data):
    """Load information about the Optzyme Groups"""
    # Store the errors here
    errors = ''
    # If the data is missing
    if "Optzyme Group" not in data:
        errors += "\nThe Optzyme Group information is missing"
    else:
        # Make sure that the data is a list
        if isinstance(data["Optzyme Group"], str):
            data["Optzyme Group"] = [data["Optzyme Group"]]
        # Store the Optzyme Groups in this list
        groups = []
        # Go through the lines
        for line in data["Optzyme Group"]:
            # Should be organized as 
            # objective property (with weight of #) using state Molecules: mn1
            # mn2
            items = line.split()
            if len(items) < 6 or (items[0].lower() not in ['improve', \
            'maintain', 'reduce', 'eliminate']) or (items[1].lower() \
            not in ['km', 'kcat/km', 'kcat']):
                errors += "\nThe following line is not a valid Optzyme Group "
                errors += "specification:\n" + line
                continue
            # Store the information
            group = [items[1].lower()]
            group.append(items[0].lower())
            if items[1].lower() == "kcat":
                group.append(float(items[6]))
            else:
                group.append(defaultWeight)
            # Store the Binding Assemblies
            assemblies = []
            # First store the TSA state Binding Assembly, if one exists
            if "TSA" in line:
                i = items.index("TSA")
                # The objective for a TSA state Binding Assembly is 'improve'
                assembly = ['improve']
                for item in items[i+2:]:
                    if len(item) == 1:
                        assembly.append(item)
                    elif item == "and":
                        break
                assemblies.append(assembly)
            # Next store the ground state Binding Assembly, if applicable        
            if "ground" in line:
                i = items.index("ground")
                # The objective for a ground state Binding Assembly is
                # 'eliminate'
                assembly = ['eliminate']
                for item in items[i+2:]:
                    if len(item) == 1:
                        ...
                        break
                assemblies.append(assembly)           
            # Store the assemblies in the group
            group.append(assemblies)
            # Store the group 
            groups.append(group)
        # Store the Optzyme Groups
        try:
