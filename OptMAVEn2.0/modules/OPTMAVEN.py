...
#mfa#
def initialize_antigen(experiment):
    """ Add missing atoms to the structure of the antigen, relax the
    structure, generate the PSF and PDB of the antigen, and move the
    antigen to the initial position. """
    # Generate a PSF for each molecule in the experiment.
    KS.generate_PSFs(experiment)
    # Perform an energy minimization on each molecule.
    KS.relax_molecules(experiment)
    ...
    KS.initial_antigen_positions(experiment)
...
def regroup_energies(experiment):
    ...
    positions = [position_str(map(float, line.split())) for line in
            open(positions_file)]
    # Get all of the MAPs parts.
    parts = MAPs.list_parts()
    ...
    for molecule in os.listdir(energy_dir):
        ...
        for part in parts:
            ...
            if not os.path.isfile(energy_file):
                raise ValueError("\nWARNING: Missing energies for part {} in "
                        "molecule {}.".format(part, molecule))
        ...
        for position in positions:
            position_energies = energies[position]
            energy_file = os.path.join(mol_dir, "{}.dat".format(position))
            open(energy_file, "w").write("\n".join(["{} {}".format(part, energy)
                    for part, energy in position_energies.iteritems()]))
        # Remove the former energy files to save disk space.
        for part in parts:
            try:
                os.system("rm -r {}".format(os.path.join(mol_dir, part)))
            except OSError:
                pass

def antibody_coordinates(d, gap):
    # First get the xyz coordinates of the epitope.
    coordinates = [d["x"], d["y"], d["z"]]
    # Then add the rotation angle: use the sine-cosine representation.
    coordinates.extend([np.cos(d["zRot"]), np.sin(d["zRot"])])
    # Then add the virtual coordinates of the MAPs parts it comprises.
    for domain, region in itertools.product("HL", ("V", "J", "CDR3")):
        category = domain + region
        part = d[category]
        part_coords = MAPs.get_coordinates(part, gap)
        coordinates.extend(part_coords)
    return np.array(coordinates)

def cluster_designs(experiment):
    """ Cluster the designs and generate the antibody/antigen complex of the best ones. """
    performance_file = PERFORMANCE.performance_file(experiment)
    timer = PERFORMANCE.Timer() 
    timer.start() 
    folder = experiment["Folder"]
    ie_directory = os.path.join(folder, "energies")
    for antigen in os.listdir(ie_directory):
        parts_all = os.path.join(ie_directory, antigen, "parts_all.csv")
        if not os.path.isfile(parts_all):
            raise OSError("The file of all designs does not exist.")
        fields = ["zRot",   "x",   "y",   "z", "cut", "energy", "HV", "HJ",
                "HCDR3", "LV", "LJ", "LCDR3"]
        dtypes = [ float, float, float, float,   int,    float,  str,  str,
                    str,  str,  str,     str]
        designs = list()
        with open(parts_all) as f:
            f.readline()
            for line in f:
                designs.append(OrderedDict())
                cells = [c.strip() for c in line.split(",")]
                # Position and energy
                for i in range(6):
                    designs[-1][fields[i]] = dtypes[i](cells[i])
                # Heavy chain parts
                for i in range(6, 9):
                    designs[-1][fields[i]] = dtypes[i]("{}_{}".format(fields[i],
                            cells[i]))
                # Light chain parts
                light_chain = None
                for i in range(9, 12):
                    kappa = cells[i]
                    lambda_ = cells[i + 3]
                    if kappa != "" and lambda_ != "":
                        raise ValueError("Mix of kappa and lambda parts.")
                    if kappa != "" and light_chain != "L":
                        light_chain = "K"
                        part = fields[i].replace("L", "K")
                        number = kappa
                    elif lambda_ != "" and light_chain != "K":
                        light_chain = "L"
                        part = fields[i]
                        number = lambda_
                    else:
                        raise ValueError("Missing light chain part.")
                    designs[-1][fields[i]] = dtypes[i]("{}_{}".format(part,
                            number))
        dtype_map = {int: "i4", float: "f4", str: "S16"}
        design_dtypes = [(field, dtype_map[t]) for field, t in zip(fields, dtypes)]
        designs = np.array([tuple(design.values()) for design in designs],
                dtype=design_dtypes)
        # Generate coordinates for each design.
        coordinates = OrderedDict([((d["zRot"], d["x"], d["y"], d["z"], d["cut"]),
                antibody_coordinates(d, 8)) for d in designs])
        # Cluster the antibodies.
        cluster_sets = OrderedDict()
        k = 1
        k_max = 8
        best_k = None
        best_std = None
        ...
        while k <= k_max:
            print k
            clusters = KMEANS.kmeans(coordinates, k, None)
            centers = [np.mean(cluster.values(), axis=0) for cluster in clusters]
            std = max([sum([np.linalg.norm(member - center) for member in cluster.values()]) for cluster, center in zip(clusters, centers)])
            print std
            print "---"
            cluster_sets[k] = (clusters, std)
            if best_std is None or std < best_std:
                best_std = std
                best_k = k
            k += 1
        open(os.path.join(experiment["Folder"], "results", "k-means.dat"), "w").write("\n".join(["k\tstd"] + ["{}\t{}".format(k, std) for k, (clusters, std) in cluster_sets.iteritems()]))
        #FIXME implement the SCRE plot. For now, just choose the k with the smallest std.
        best_k = best_k
        clusters, std = cluster_sets[best_k]
        cluster_file = os.path.join(experiment["Folder"], "results", "clusters.txt")
        open(cluster_file, "w").write("\n\n".join(["Cluster {}\n{}".format(i + 1, "\n".join(map(format, cluster.keys()))) for i, cluster in enumerate(clusters)]))
        performance_file.sub_time("clustering", timer)
        # Assemble the 2 antibodies in each cluster with the strongest binding.
        timer = PERFORMANCE.Timer()
        timer.start()
        antibodies = list()
        total_antibodies_min = 1 #FIXME: this is just for benchmarking so that the antibodies don't occupy too much space. Set to 50 or 60 in practice.
        antibodies_per_cluster = int(np.ceil(total_antibodies_min / float(best_k)))
        # Index all of the antibody designs by their position and cut.
        design_info = OrderedDict([(tuple([d[field] for field in fields[0: 5]]),
                tuple([d[field] for field in fields[5: 12]])) for d in designs])
        for i, cluster in enumerate(clusters):
            cluster_info = [OrderedDict(zip(fields, list(design) + list(design_info[design]))) for design in cluster.keys()]
            [design.update({"cluster": i + 1}) for design in cluster_info]
            best_designs = sorted(cluster_info, key=lambda x: x["energy"])[0: antibodies_per_cluster]
            antibodies.extend(best_designs)
        # Assemble the antibodies. Relax the complex with the most negative
        # pre-relaxation interaction energy.
        KS.assemble_antibody_antigen_complexes(experiment, antibodies, False)  #FIXME: change False to 1 for real
        performance_file.sub_time("antibody_complex_assembly_and_relaxation", timer)
...
#mfa#
def select_all_parts(experiment, batch_size=1):
    """ Select parts for all antigen positions in an experiment. """
    positions_file = os.path.join(experiment["Folder"], "input_files", "positions.dat")
    positions = [position_str(map(float, line.split())) for line in
            open(positions_file)]
    n_pos = len(positions)
    energy_dir = os.path.join(experiment["Folder"], "energies")
    ...
    for molecule in os.listdir(energy_dir):
        ...
        energy_files = [os.path.join(mol_dir, "{}.dat".format(position))
                for position in positions]
        for first in range(1, n_pos + 1, batch_size):
            last = min(first + batch_size - 1, n_pos)
            pos_range = "positions_{}_to_{}".format(first, last)
            commands = ["date"]
            for position in positions[first - 1: last]:
                position_file = os.path.join(mol_dir, "{}.dat".format(position))
                ...
                commands.append("python {} {}".format(os.path.join(
                        ...
                        position_file))
                # Delete the file of energies at the position to save disk space.
                commands.append("rm {}".format(position_file))
            ...
            command = "\n".join(commands)
            name = os.path.join(experiment["Folder"], "{}_MILP.sh".format(
                    os.path.splitext(pos_range)[0]))
            SUBMITTER.experiment_script(experiment, name, command,
                    time_="MILP_for_{}".format(
                    pos_range), self_destruct=True)
