...
To begin, please provide some basic information about your experiment."""
    ...
    for_name(experiment)
    #mfa#
    # IF the "experiments" folder does not exist, create it.
    exp_dir = os.path.join(InstallFolder, "experiments")
    if not os.path.isdir(exp_dir):
        os.mkdir(exp_dir)
    # Store the Experiment's folder
    #mfa#
    experiment["Folder"] = os.path.join(exp_dir, experiment["Name"]) + "/"
...
#mfa#
def for_OPTMAVEN_grid(experiment, default=True):
    """ Get the settings for the OptMAVEn grid search. """
    defzr = [0, 300, 6]
    defx = [-5, 10, 7]
    defy = [-10, 5, 7]
    defz = [3.75, 16.25, 11]
    defgrid = [defzr, defx, defy, defz]
    grid_range = None
    if default:
        if FUNCTIONS.answer_question("Would you like to use the default"
                " grid settings for OptMAVEn? ", "bool"):
            grid_range = defgrid
    if grid_range is None:
        grid_range = list()
        for what, default in zip(["z rotation angle", "x center", "y center",
                "z center"], defgrid):
            answer = None
            while answer is None:
                # Ask the user to define the grid settings.
                answer = [FUNCTIONS.answer_question("What should the {} {} be?"
                        " The default is {}. ".format(spec, what, defspec),
                        "float") for spec, defspec in zip(["minimum", "maximum",
                        "step"], default)]
                # Make sure the minimum is not greater than the maximum and the
                # step is positive.
                if answer[0] > answer[1] or answer[2] <= 0:
                    print("The minimum {} must not be greater than the maximum,"
                             " and the step must be positive.".format(what))
                    answer = None
                # The z rotation angle must be within the range [0, 360).
                elif what == "z rotation angle" and (answer[0] < 0 or answer[1]
                        >= 360):
                    print "The {} must be >= 0 and < 360.".format(what)
                    answer = None
                # Make sure the range is (nearly) a multiple of the step size.
                ratio = (answer[1] - answer[0]) / answer[2]
                eps = 0.001
                if ratio - int(ratio) > eps and ratio - int(ratio) < 1 - eps:
                    print "The difference between the maximum and minimum {} "\
                        "must be an integer multiple of the step.".format(what)
                    answer = None
            grid_range.append((answer[0], answer[1], int(round(ratio)) + 1))
    experiment["Optmaven Grid"] = [np.linspace(*what) for what in grid_range]
    print experiment["Optmaven Grid"]


def OptMaven_info(experiment): #mfa#
...
Please refer to the provided documentation for additional information.
    ...
    #for_annealing_sharing(experiment, default)
    #for_energy_type(experiment, default)
    Epitope(experiment) #mfa#
    for_OPTMAVEN_grid(experiment, default) #mfa#
