...
def regroup_energies(experiment):
    ...
    for part_type in [d for d in os.listdir(MAPs_dir) if d.startswith((
            "H", "K", "L"))]:
        part_dir = os.path.join(MAPs_dir, part_type)
        parts.extend([os.path.splitext(part)[0] for part in os.listdir(
                part_dir) if part.endswith(".pdb")])
    ...
    for molecule in os.listdir(energy_dir):
        ...
        for part in parts:
            ...
            if not os.path.isfile(energy_file):
                open(os.path.join(experiment["Folder"], "performance.txt"),
