...
def relax_molecule(experiment, molecule):
    ...
    iteration = 0
    total_time = 0.0
    timer = PERFORMANCE.Timer(os.path.join(experiment["Folder"], "performance.txt"))
    timer.start("NAMD relaxation of molecule {}".format(molecule_coords))
    ...
    timer.stop()
    timer.dump()
    open(os.path.join(experiment["Folder"], "performance.txt"), "a").write(
            "\nAfter relaxation, energy is {} kcal/mol\n".format(
            running_energy[-1]))
...
def initial_antigen_position(experiment, molecule):
    ...
    timer = PERFORMANCE.Timer(os.path.join(experiment["Folder"],
            "performance.txt"))
    timer.start("Mounting antigen {}".format(name))
    ...
    open(os.path.join(experiment["Folder"], "performance.txt"), "a").write(
            "\nAfter mounting antigen:\n" + open(final_centers).read())
    try:
...
def cull_clash(experiment, molecule):
    ...
    timer = PERFORMANCE.Timer(os.path.join(experiment["Folder"],
            "performance.txt"))
    timer.start("Culling clashes for molecule {}".format(Ag))
    # Run VMD.
    ...
    if not os.path.isfile(posFile):
        raise Exception("VMD failed to generate a file of the non-clashing "
                "positions.")
    # Ensure that there is at least one non-clashing position.
    num_positions = len(open(posFile).readlines())
    if num_positions == 0:
        raise Exception("There were no positions that did not clash. Please "
                "re-examine the structure of the antigen and adjust your grid "
                "settings.")
    # Find the disk usage after antigen positioning.
    du_out = os.path.join(experiment["Folder"], "du.out")
    while os.path.isfile(du_out):
        du_out = "1" + du_out
    os.system("du -sh {} > {}".format(experiment["Folder"], du_out))
    du = open(du_out).read()
    try:
    	os.remove(du_out)
    except OSError:
        pass
    open(os.path.join(experiment["Folder"], "performance.txt"), "a").write(
            "\nNumber of positions found for molecule {}: {}\nDisk usage after "
            "finding positions: {}\n".format(molecule.generate_name(),
            num_positions, du))
    ...
    for antigen in [mol[2] for mol in experiment["Molecules"]]:
        ...
        for Mtype, parts in MAPs_parts.iteritems():
            for part in parts:
                ...
                commands = ["date", "cd {}".format(part_directory)]
